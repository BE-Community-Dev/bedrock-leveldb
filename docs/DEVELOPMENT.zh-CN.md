...
本 crate 对 native LevelDB 采取读优先定位。native manifest、WAL 和 table
文件会被直接读取。本 crate 提供的写入能力只面向本地工具场景：flush 后的数据
使用 crate 自定义的 `BWLDB...` table 和 manifest 格式。除非已经实现并测试了
native 兼容写入，否则不要把自定义写入描述成 native LevelDB 兼容输出。
...
- `manifest.rs`：native/custom manifest 解析，以及用于 range filtering 的 table
  metadata。
- `table.rs`：table footer、index/data block 读取、restart array、解压、
  internal key 处理和 custom table 写入。
...
写 WAL record 或创建 custom table。任何新增的写入路径都必须检查 read-only 行为，
...
- custom flush/reopen 保留 key、value 和 deletion。
...
- Overlay hot path。
- Flushed custom table point read。
